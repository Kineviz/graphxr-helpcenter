= Kineviz (GraphXR) and Kineviz Desktop Release History

[cols="1,1,10"]
|===
|*Tag* |*Date* |*Release Highlights*

|xref:./gxr-3-22-0-release-notes.adoc[3.22.0] |7/13/2026 
a|* *Usability*
** *NEW / RENAMED PRODUCT LINE*: *Kineviz* (formerly *GraphXR*) and *Kineviz Desktop*
** Live status bar at the bottom of an open project window.
** *Agent* enhancements: Flexible panel UI, *_Export as HTML_*.
** *Export* or *Import* an entire project.
** Re-design *_Views_* panel and folders for flexible panel layout.
** Project cloning or duplication copies views and internal data.
** Improved Expand, Query bar, and alerts for Transforms. 

* *Performance* 
** Improved UI and KoreDB performance.   

* *Admin, Security, Configuration, and Deployment*

** *Agent Data Connector (ADC)* implemented for BigQuery and Spanner.
** *Kineviz Desktop* v.0.14.0 (beta) access, payment system, upgrade path, version usage, and agent feedback implemented through *Kineviz Cloud*.  

* *Extensions* 
** *Grove*, *File* manager, *Text Editor*

|xref:./gxr-3-21-0-release-notes.adoc[3.21.0] |5/21/2026 
a|* *Usability*
** *_Agent_* enhancements: 
*** *_Thread History_* to search and select threads
*** Optional *_Connector_* tab in Agent *_Settings_* to manage installed connectors.

** Create View folders on the *View* menu to organize and save views for manual or agentic work within a project.

* *Admin, Security, Configuration, and Deployment*
** The internal datastore is renamed *KoreDB* (originally *Kuzu DB*; now based on the open-source *ladybug* DB).

* *Kineviz Desktop* v.0.1.7 (alpha)
** A *Kineviz Desktop* application enables local access to GraphXR functionality. Upon login, the AI-powered *GraphXR Agent* is optionally connected to managed AI models via a secure *Kineviz Cloud* backend.

* *Extensions* 
** *Grove*, *File* manager, *Text Editor*

|xref:./gxr-3-20-0-release-notes.adoc[3.20.0] |5/9/2026 
a|* *Usability*
** *GraphXR Agent* (beta), for AI-enabled engagement with graph visualization and analytics.
** Floating panels and tabs and flexible docking enable a customizable project user interface.
** *Expand All* can be performed in one step, rather than 1000 nodes at a time.
** Views can be selected for inclusion in a cloned project.
** More than one view can be deleted at a time.
** The Other (Kineviz logo) menu lists the logged in user. 
** A *Help* button is added to the left menu. 
** The *Query* bar now saves recent queries in a *Query History* menu.

* *Admin, Security, Configuration, and Deployment*
** The internal datastore has been migrated from *Kuzu* to a *ladybug* DB.
** Grovebook auto-open behavior is now aware of project sandbox settings.

* *Performance* 
** Speed of loading from Neo4j is increased.

* *GraphXR API* 
** gxr.agent.createSession(). 
** gxr.agent.setActiveSessionId(sessionId).
** gxr.views.copyToProjectFiles({id:"theViewID"})
** gxr.database() is removed to prevent confusion with gxr.query().  

|xref:./gxr-3-19-0-release-notes.adoc[3.19.0] |4/29/2026 
a|* *Usability*
** Interactive *Minimap* added to the project canvas to orient the user.
** Other menu (Kineviz logo) includes *Create New Project* item.

* *Performance* 
** Speed and response of panning and camera reset is improved.

|xref:./gxr-3-18-0-release-notes.adoc[3.18.0] |4/22/2026 
a|* *Usability*
** Query bar accepts open-ended pattern, and user-selected limit on results.
** Map panel boundaries are now visible.
** A parametric layout is cleared before a Map layout is applied.
** In the Other menu (Kineviz logo), Cmd or Ctrl - Click to open a menu selection in a separate tab  

|xref:./gxr-3-17-0-release-notes.adoc[3.17.0] |4/1/2026 
a|* *Usability*
** Legend labeled with icons, not text.
** Shared projects include a blue dot in the Projects list.

* *GraphXR API* 
** gxr.invertSelectedNodes()

|xref:./gxr-3-16-0-release-notes.adoc[3.16.0] |3/17/2026 
a|* *Usability*
** Views menu includes Rename, Duplicate, Share, and Delete a view.
** Shared projects include a blue dot in the Projects list.

* *Performance* 
** Speed of hiding nodes in large graphs accelerated.

* *GraphXR API* 
** gxr.expandFromDatabase()

|xref:./gxr-3-15-0-release-notes.adoc[3.15.0] |3/4/2026 
a|* *Usability*
** Extract transform UI is improved

* *Performance* 
** Speed of path finding in large graphs accelerated.

* *GraphXR API* 
** gxr.views.save, gxr.views.list, and gxr.views.load
** gxr.files.get(path) 

|xref:./gxr-3-14-0-release-notes.adoc[3.14.0] |2/11/2026 
a|* *Usability*
** Transform panels and Legend refactored with antD components and new UI.
** *Export* menu at the right of the Legend more compact.
** Query bar property filter improved

* *Admin, Security, Configuration, and Deployment*
** API key for database proxy.
** Per-user API keys

* *GraphXR API* 
** gxr.relationshipSelect() 
** gxr.categorySelect()

|xref:./gxr-3-13-0-release-notes.adoc[3.13.0] |1/6/2026 
a|* *Usability*
** *Recent Projects* item added to *More* menu.

|xref:./gxr-3-12-0-release-notes.adoc[3.12.0] |12/17/2025 
a|* *Usability*
** Restore UI Configuration to basic Settings.

* *Performance* 
** Reduced memory usage on startup.
 
* *Admin, Security, Configuration, and Deployment*
** Enable signup by default on first boot.
** MIT: Improve SAML login.

|xref:./gxr-3-11-0-release-notes.adoc[3.11.0] |12/10/2025 
a|* *Usability*
** *DataStore* UI enhancements for clarity and safer database deletion.
** *Home* and *Logout* confirmation modals improved.
** Query Bar now surrounds property identifiers with backticks.
 
* *Admin, Security, Configuration, and Deployment*
** User consent modal added to PostHog session replay.

* *GraphXR API* 
** Deprecated gxr.add()

* *Extensions* 
** Enforce extension version compatibility.
** Support for installing extensions and Grove notebooks via configuration and environment variables.
...
|xref:./gxr-2-13-0-release-notes.adoc[2.13.0]|2/7/2022 
a|* Collection Nodes
* Full 2D mode
* Screen capture tool
* Edge captions
...
|2.2.3 |2/14/2020 a|* Add multiple caption support
...
* Support multiple tags for the same nodes
