...
The *_Query > SQL_* subpanel lets you run SQL queries and model the results as graph data. You can:
...
+
IMPORTANT: Query results can only be loaded to the graph if they are in the form of a table of values with column headings. Some SQL queries do not return such results.
+

*To run a SQL query and import the results:*

. Open the *_Query > SQL_* subpanel.
...
+
image::/v3/02_03_12_SQLShowTable.png[,520,380,role=text-left]
+
...
+
image::/v3/02_03_13_SQLResults.png[,520,380,role=text-left]
+
...
+
image::/v3/02_03_14_SQLResultsLoaded.png[,720,480,role=text-left]
+

+
NOTE: Use the xref:./import-via-mapping.adoc[*_Mapping Editor_*] to create a *_New_* mapping or *_Apply_* or *_Edit_* an existing one. You can also import a mapping saved from another project. 
