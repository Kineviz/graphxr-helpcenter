= On-Prem Kineviz Installation

For Enterprise users working with Kineviz in a secure environment, on-premises installation options include:
 
* Installation using Docker
* Install with a Docker Zip file
* Install with persistent storage
* Update or re-install Kineviz

NOTE: Individual Kineviz users can simply go to the browser, create an account and log in. Installation as such is not needed.

== Basic Docker Install

*To install Kineviz from docker:*
...
docker login --username=<username>  --password=<password>
....
. Pull the Kineviz image (i.e. the executable).
+
....
...
. Run the docker container with the Kineviz executable.
+
....
...
+
On completion, the Kineviz server is running and has established your Admin +
account, which is now ready for configuration. 
+

NOTE: For security, Kineviz sets cookie security to _true_.
...
*To install Kineviz from Docker using a .zip file:*

. Pull the image from docker hub.
+
....
docker pull kineviz/Kineviz_custom:develop
....
...
*To install Kineviz with persistent storage:*
...
Use the following commands to install and run the Kineviz demo with persistent storage.
