= Installing WebXR Virtual Reality for Kineviz

Kineviz supports use in a Virtual Reality (VR) or Augmented Reality (AR) environment through WebXR and an Oculus headset.

This guide outlines setting up an environment for using Kineviz with WebXR, in this case, using the Oculus Rift, Oculus Rift-S, and Oculus Quest 2 (via Oculus Link) headsets with Chrome WebXR. The steps are:
...
. Load Data in to _Kineviz_, for example using a Cypher query as follows:
