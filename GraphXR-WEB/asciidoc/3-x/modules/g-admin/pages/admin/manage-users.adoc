...
The *_Users_* panel displays the list of current users and lets you review and manage their access to Kineviz and to the connected databases.

image::/v3/01_01_AdminOpen.png[,720,480,role=text-left] 
...
When an email server has been configured, each Kineviz user must have a user account. A user who does not already have an account is prompted for a user name and password when logging in.

If no email server is configured, users only need a url to access Kineviz. This can provide shared use or a read-only account.
...
+
image::/v3/02_02_AddUserButton.png[,720,420,role=text-left]
+ 

. Click the *_Add User_* button at the top right, and enter the following information:
+
image::/v3/02_03_AddUserDialog.png[,720,420,role=text-left] 
+

* *_Email_*: The email used to sign in to the account.
* *_First Name_* and *_Last Name_*: The user's first and last name.
* *_Password_*: A password you or the user selects. Upon signing in to Kineviz, a user can reset the password.
* *_Force Change Password_* checkbox: Selected by default, so that the user is required to change their password at specific intervals. 
* *_Tags_*: Enter a version tag to grant access to specific pre-release versions of Kineviz.
* *_Plan Days_*: Indicates the number of days remaining on the user's subscription plan.
* *_Access_*: Set to _Active_, _Inactive_, _Reset_, or _Freeze_.
* *_UserType_*: Set as _Normal_ or _Admin_.
...
+
* *_Connector API Auth_*: Click  *_Disable_* or *_Enable_* to grant access to the third party *_Connector_* applications in a Kineviz Enterprise installation.
* *_API Key Access_*: Click  *_Disable_* or *_Enable_* to grant access to the Kineviz API via a key.
+
...
+
A message at the top of the *_Add User_* dialog indicates that the user was successfully added. You can keep adding more users until you *_Cancel_* or close the dialog manually.
...
image::/v3/02_03_UserSearch.png[,720,420,role=text-left]
...
+
image::/v3/02_04_ExportProfiles.png[,720,480,role=text-left] 
+
...
+
image::/v3/02_05_DBAuth.png[,720,480,role=text-left]
+ 
