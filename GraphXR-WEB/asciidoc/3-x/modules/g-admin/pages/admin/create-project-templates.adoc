...
A template saves the current settings in a project, including node and edge sizes,  captions, colors, and icons, as well as *_UI configuration_* options that control which interface elements are shown or hidden in the project. A user can apply a template to any new or existing project, replicating its settings in one step.

A template can be created by :

* A general user within a project using the *_Create Template_* button in the *_Project > Data Store_* subpanel.
* An admin user in the *_Templates_* panel. 

As soon as it is created, the template appears on the template menu available when creating or updating a Kineviz project.

image::/v3/08_02_ProjectTemplates_Set.png[,420,420,role=text-left]

In the *_Templates_* panel, an admin user can configure and manage the entire set of  templates at the server level. 
 
image::/v3/08_01_ProjectTemplates_Start.png[,720,480,role=text-left]

== Add or Delete a template

You can add a new template at any time. You can either create a template with a default configuration which you can then edit, or upload a template .JSON file.

*To add a template:*

. Click *_Add Template_*, and provide a name and description.
+
image::/v3/08_03_ProjectTemplates_Add.png[,720,340,role=text-left]
+
. Either:
+
...
+
image::/v3/08_04_ProjectTemplates_Delete.png[,720,340,role=text-left]
+
...
+
image::/v3/08_05_ProjectTemplates_Edit.png[]

=== Configure a template

Template configuration is organized in separate tabs:

image::/v3/08_06_ProjectTemplates_Configure.png[,720,340,role=text-left]
...
+
image::/v3/08_07_ProjectTemplates_HideAddExtension.png[,720,420,role=text-left]
+
...
+
image::/v3/08_08_ProjectTemplates_NewMenu.png[,720,720,role=text-left]
...
+
image::/v3/08_09_ProjectTemplates_CatRel.png[,720,480,role=text-left] 
+
...
+
image::/v3/08_11_TemplatePriority.png[,720,480,role=text-left]
...
+
image::/v3/08_12_TemplateDownload.png[,720,280,role=text-left]
